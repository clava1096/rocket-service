	...
	default:
		...
		case '/': // Prefix: "/api/v1/orders"
			...
			if len(elem) == 0 {
				switch r.Method {
				case "POST":
					s.handleCreateOrderRequestRequest([0]string{}, elemIsEscaped, w, r)
				default:
			...
			case '/': // Prefix: "/"
				...
				if len(elem) == 0 {
					switch r.Method {
					case "GET":
						s.handleGetInfoOrderByUUIDRequest([1]string{
							args[0],
						}, elemIsEscaped, w, r)
					default:
	...
	default:
		...
		case '/': // Prefix: "/api/v1/orders"
			...
			if len(elem) == 0 {
				switch method {
				case "POST":
					r.name = CreateOrderRequestOperation
					r.summary = "Создает новый заказ"
					r.operationID = "CreateOrderRequest"
			...
			case '/': // Prefix: "/"
				...
				if len(elem) == 0 {
					switch method {
					case "GET":
						r.name = GetInfoOrderByUUIDOperation
						r.summary = "Получить информацию о заказе по UUID"
						r.operationID = "GetInfoOrderByUUID"
